...
void _get_distances(int sites[],
    ...
    */
    ...
    int idx = 0;
    //std::vector<double> xyz_i;
    //std::vector<double> xyz_j;
    int codon_i, codon_j;
    
    for (int i=0; i < len; i++) {
      //codon_i = t.get_codon_number_for_cds_position(sites[i]);
      //xyz_i = three_d_locations[codon_i];
      //xyz_i = three_d_locations[floor((float)sites[i] / 3)];
      codon_i = floor((float)sites[i] / 3);
      for (int j=i+1; j < len; j++) {
	codon_j = floor((float)sites[j] / 3);
	//codon_j = t.get_codon_number_for_cds_position(sites[j]);
	//xyz_j = three_d_locations[codon_j];
	//xyz_j = three_d_locations[floor((float)sites[j] / 3)];
	//distances[idx] = std::pow((xyz_i[0] - xyz_j[0])*(xyz_i[0] - xyz_j[0]) +
	//			  (xyz_i[1] - xyz_j[1])*(xyz_i[1] - xyz_j[1]) +
	//			  (xyz_i[2] - xyz_j[2])*(xyz_i[2] - xyz_j[2]) , .5);
...
void _get_distances(const std::vector<int> & sites, std::vector<int> & distances) {
    int size = sites.size();
    int len = ((size - 1) * size) / 2;
    int sites_array[size];
    for (int i=0; i<size; i++) {
...
double _geomean(double distances[], int & len) {
    /**
        gets the geometric mean of a vector of distances
