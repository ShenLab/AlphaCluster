...
#run poisson test
python denovonear/__main__.py poisson --in data/$disease.power_cohort_$tag/$SLURM_JOB_ID.cohort_size_$size.$disease.released.hg38.txt --N $size --protein $gene --out data/$disease.power_cohort_$tag/$SLURM_JOB_ID.poisson_pvalue.txt  --gencode data/gencode.v38.annotation.gtf.gz --fasta data/genome.hg38rg.fa
